...
class MongoDB:
    ...
    def delete_from_array(
            self, collection: str, query: Dict, update_data: Dict) -> bool:
        """Remove specified element from specified array field in queried document."""
        return self.collection(collection).update_one(
            query, {"$pull": update_data,
                    "$set": self.timestamp({}, "updated_at")}).acknowledged

    def upsert_to_set_array(
            self, collection: str, query: Dict, update_data: Dict) -> bool:
        """Create new document or add only new values to document with specified array field."""
        return self.collection(collection).update_one(
    ...
    def count(self, collection: str, query: Optional[Dict] = None) -> int:
        """Return the number of documents in collection that match query."""
    ...
    def create_index(self, collection: str):
        """Internal only! Create an index within the given collection."""
        self.collection(collection).create_index([("$**", "text")])

    def drop_index(self, collection: str):
        """Internal only! Remove existing index from the given collection."""
    ...
    def get_database_info(self):
        """Return all collection names and their doc counts."""
        return {
