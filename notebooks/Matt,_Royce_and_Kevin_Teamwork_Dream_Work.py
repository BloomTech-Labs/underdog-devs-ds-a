...
convictions = (
    "Misdemeanors", "Felony", "Wobbler",
)

skills = (
    "HTML", "CSS", "JavaScript", "Python", "Ruby",
    "C++", "Axios", "React", "Node.JS", "C#", "Django",
    "MongoDB", "SQL", "PostgreSQL", "NoSQL", "PHP",
    "Angular", "Microsoft Azure", "IOS", "Android"
)
...
class Mentee:
    ...
    def __init__(self):
        ...
        self.full_name_mentee = self.first_name + " " + self.last_name
        self.time_zone = r.sample(time_zones, k=1)

        self.email = self.profile_id + self.last_name.lower() + self.email_end
...
class Mentor:
    ...
    def __init__(self):
        self.first_name = r.choice(male_first_name)
        self.last_name = r.choice(last_name)
        self.full_name_mentor = self.first_name + " " + self.last_name
        self.profile_id = "O" + str(hash(r.randint(2000000, 70000000000000000)))
        
        self.skills = sorted(r.sample(skills, k=r.randint(3, 6)))
        self.tech_or_career = r.sample(purpose, k=r.randint(1, 2))
...
# + colab={"base_uri": "https://localhost:8080/", "height": 1000} id="eLSEvUAPFuir" outputId="6973654b-eadb-424e-ffe6-b5e303adc5b5"
mentor_df

# + colab={"base_uri": "https://localhost:8080/", "height": 423} id="zI-m9SusFwWp" outputId="1f1f9c40-f091-4519-9e16-7668ce7a1bce"
mentee_df

# + [markdown] id="ZpA1adiB09a1"
# Key Features (mentee): experience, tech_or_career, desired_skils, time_zone
#
#
# Key Features (mentor); skill_rank, career_capable, skills, time_zone

# + [markdown] id="t6KTW-ib1jyG"
#
...
def clean_data(df):
    ...
    df = pd.concat([pd.get_dummies(df[c].apply(pd.Series).stack()).sum(level=0) for c in columns_needed], axis=1)
    return df


# + colab={"base_uri": "https://localhost:8080/"} id="fVme8fpmInuz" outputId="781a717c-4137-4bd1-fbc7-e5bae71dcc00"
clean_data(mentor_df).shape

# + colab={"base_uri": "https://localhost:8080/"} id="ocUTua6MGKTd" outputId="38a48f5c-bc4b-4f82-b282-aee3855bb069"
clean_data(mentee_df).shape

# + id="PnE_ay_hE0zg" colab={"base_uri": "https://localhost:8080/"} outputId="158beb81-9ad7-4cbe-9115-54199567fa44"
cos_model_df = cosine_similarity(np.array(clean_data(mentee_df).iloc[0]).reshape(1,-1),clean_data(mentor_df))

# + id="1iQrEcD-11fz" colab={"base_uri": "https://localhost:8080/"} outputId="3ce65840-12be-4bc7-c0cd-12cdaac48a22"
cos_model_df.shape

# + id="EAUVbLXX6p53"
cos_model_df = pd.DataFrame(cos_model_df,columns=mentor_df.full_name_mentor)

# + colab={"base_uri": "https://localhost:8080/"} id="3Ico9_ptL7ga" outputId="ff15f01e-1aff-43a9-e7f6-de1e6b8c1b28"
cos_model_df.mean()

# + colab={"base_uri": "https://localhost:8080/", "height": 1000} id="zg6uVrYa2mN6" outputId="81f071eb-dd43-4982-e769-ba61ff54ed13"
cos_model_df = cos_model_df.unstack().reset_index().drop(columns = ['level_1']).rename(columns = {0: 'cos_sim'})
cos_model_df

# + colab={"base_uri": "https://localhost:8080/", "height": 206} id="AtB8vBP4NK9d" outputId="65af23d5-08b4-4a33-d98f-d6d393c7e196"
recomendation = pd.concat([cos_model_df, mentor_df], axis = 1, join = 'inner').sort_values(by = 'cos_sim', ascending = False).drop(columns = ['first_name', 'last_name'])
recomendation.head()

# + [markdown] id="qNqiHzTfzltI"
# # Testing a method of preening data down to list that only contains mentors that teach all skills the mentee is looking for

# + colab={"base_uri": "https://localhost:8080/"} id="Mmov_ozpQzve" outputId="6aecf75b-eebd-4d62-b9c4-71fa80830824"
skill_list = mentee_df.iloc[0]['skills']
skill_list


# + id="DpUUhJwjyvqe"
def preen(outer, inner):
  encompass = True
  for skill in inner:
    if skill not in outer:
      encompass = False
  return encompass


# + colab={"base_uri": "https://localhost:8080/"} id="5ZEkvQI1tezx" outputId="3a972565-82a4-476b-e12a-55bded43f6ee"
filtered = recomendation[recomendation['skills'].apply(lambda x: preen(x, skill_list))]
filtered.shape


# + [markdown] id="W5fZEc8S0Cth"
# # Fails because there are no mentors that have all skills listed

# + [markdown] id="f5W2-qF00NX9"
# # Add a new scoring system, then recomend

# + id="KJM2rVds0ZMY"
def score(outer, inner):
  count = 0
  for skill in inner:
    if skill in outer:
      count += 1
...
# + colab={"base_uri": "https://localhost:8080/", "height": 206} id="FTw9j70xO4dq" outputId="d0325747-1029-4c54-c022-56880821ee2d"
recomendation['overlap'] = recomendation['skills'].apply(lambda x: score(x, skill_list))
recomendation.head()

# + id="CCmR6xW-9uMJ" colab={"base_uri": "https://localhost:8080/", "height": 206} outputId="f6f04419-7d60-44a9-d06c-0f11de02bb5f"
recomendation.sort_values(['overlap', 'cos_sim'], ascending = [False, False]).head()

# + [markdown] id="54r0H3t510Pr"
# # This seems better as it gaurentees the results will be produced, though in a system where there is more logic to the skill lists in both mentors and mentees the problem likely becomes moot
