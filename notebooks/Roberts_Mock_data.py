...
"""
Time_of_day is split up into 6 hour increments. 
Morning is 6AM - 12PM
Afternoon is 12PM - 6PM
Evening is 6PM - 12AM
"""
...
goals = (
    "I want to get a job in tech", 
    "I want to hone my skills", 
    "I want to better prepare for the interview process", 
    "I want to network"
)
...
class Mentee:
    ...
    def __init__(self):
        ...
        self.lives_in_us = bool(r.randint(0, 1))
        if self.lives_in_us:
            self.location = r.choice(state_names)
        else:
            self.location = "Other"
        self.time_zone = r.choice(time_zones)
        self.available = sorted(r.sample(time_of_day, k=r.randint(1, 3)))
        ...
        self.exp_level = r.randint(0, 5)
        self.desired_skills = r.sample(skills, k=3)

        self.your_hope = "I want a job"  # only one hope for mock data
        self.other_info = "This is other info, or none"
...
class Mentor:
    ...
    def __init__(self):
        ...
        self.mentee_load = r.randint(1, 4)
        self.career_capable = bool(r.randint(0, 1))
        self.availability = sorted(r.sample(time_of_day, k=r.randint(1, 3)))
...
# + colab={"base_uri": "https://localhost:8080/", "height": 424} id="AOPgAyEkaJQo" outputId="6ff88db0-ce46-44e5-acd4-5f26ff08733d"
mentor = Mentor()

mentor_df = mentor.to_df(200)
mentor_df

# + [markdown] id="SAGbUzcpoxdP"
# ##Total Number of Mentees
#
# The total number of generated mentees is going to be based off the total sum of mentee_load in the mentor DataFrame. I have a feeling this will give us a surplus of mentees, especially after considering all the pertinent features. 
#
# That being said, there will probably be some people that are not matched at all, the amount depends upon the random numbers created during Mentor and Mentee generation. This may be something we need to address later. 

# + colab={"base_uri": "https://localhost:8080/", "height": 783} id="sYVwE1KHXHv7" outputId="446b4c00-3b72-4f50-ec1c-a88332a0ce9e"
possible_matches = sum(mentor_df["mentee_load"])
mentee = Mentee()
mentee_df = mentee.to_df(possible_matches)
mentee_df
